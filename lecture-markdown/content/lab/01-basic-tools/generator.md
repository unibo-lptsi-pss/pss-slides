...
Oppure relativo *relativo* ad un certo punto del filesystem:
  ...
  * Ad esempio, in una struttura di cartelle in cui `A` contiene `B`;  posizionandoci su `B` e utilizzando **.** identifichiamo `B` mentre con **..** identifichiamo `A` 
...
* ZSH, Fish, e altri terminali di ultima generazione
  * Altamente personalizzabili e molto flessibili
...
* Start  $\Rightarrow$ Nella barra di ricerca, digitare `git bash`
...
1. Aprire un terminale a scelta; utilizzare gli appositi comandi per portare a termine le operazioni che seguono
1. Creare una cartella denominata `lab-01` 
1. Creare un file denominato `Hello.java` all' interno della cartella creata al passo precedente
1. Verificare che il contenuto della cartella sia quello atteso
1. Aprire con un editor di testo il file `Hello.java`
...
## Preparazione Ambiente di Lavoro II

7. All'interno del file, utilizzando l'editor, scrivere a mano il codice che segue:
</br></br>
...
8. Salvare e verificare da terminale se il contenuto del file è stato salvato correttamente
